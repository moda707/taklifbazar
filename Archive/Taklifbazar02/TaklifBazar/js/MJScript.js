...
function MobileVerification() {
    ...
    var url = "http://localhost:23524/Ajax/SendSMS$!$Verification$!$" + N.value;
...
function AddField(t, v) {
    WFFToggle("FullPanel");
    var url = "http://localhost:23524/Ajax/AddField$!$" + t + "$!$" + v;
    var xmlhttp;
    var txt, xx, x, i;
    //sending input data az url and the url will respond the out put as xml
    if (window.XMLHttpRequest) {// code for IE7+, Firefox, Chrome, Opera, Safari
...
function RemoveField(t) {
    WFFToggle("FullPanel");
    var url = "http://localhost:23524/Ajax/RemoveField$!$" + t;
    var xmlhttp;
    var txt, xx, x, i;
    //sending input data az url and the url will respond the out put as xml
    if (window.XMLHttpRequest) {// code for IE7+, Firefox, Chrome, Opera, Safari
...
function SearchTagAdd() {
    ...
    var url = "http://localhost:23524/Ajax/SearchHW$!$" + t + "$!$" + c;
...
function SearchTagRemove(t,w) {
    ...
    var c = document.getElementById("ContentPlaceHolder1_SearchShowCount").options[document.getElementById('ContentPlaceHolder1_SearchShowCount').selectedIndex].value;
    var url = "http://localhost:23524/Ajax/SearchHW$!$" + k + "$!$" + c;
...
function UP_Search(p) {
    ...
    var url = "http://localhost:23524/Ajax/UP_Search$!$" + c + "$!$" + p;
...
function UB_Search(p) {
    ...
    var url = "http://localhost:23524/Ajax/UB_Search$!$" + c + "$!$" + p;
...
function UA_Search(p) {
    ...
    var url = "http://localhost:23524/Ajax/UA_Search$!$" + c + "$!$" + p;
...
function UASR_Search(p) {
    ...
    var url = "http://localhost:23524/Ajax/UASR_Search$!$" + c + "$!$" + p;
...
function US_Search(p) {
    ...
    var url = "http://localhost:23524/Ajax/US_Search$!$" + c + "$!$" + p;
...
function RedirectDownload(t) {    
    var url = "http://localhost:23524/Download/" + t;
    window.open(url, "_blank");

}

function DoSearch() {
    var t = document.getElementById("srchterm").value.trim();
    t = t.replace(/'+'/g, ' ').replace(/-/g, ' ');

    if (t.trim() != "") {
        t = t.replace(/ /g, '-');
        location.href = "http://localhost:23524/ShowSearch/0!" + t;
    }
}
...
function ShowMore(t,v,s) {
    ...
    var url = "http://localhost:23524/Ajax/ShowMore$!$" + t + "$!$" + v + "$!$" + s;
        ...
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            if (HomeWorks.length > 0) {
                ...
                for (i = 0; i < HomeWorks.length; i++) {
                    ...
                    C += "</div><div class=\"scrollbar\" style=\"height:100px;\"><p>" + HomeWorks[i].getElementsByTagName("P_Desc")[0].firstChild.nodeValue + "</p></div></div><p><a href=\"#\" class=\"btn btn-primary\">خرید</a> <a href=\"View/" + HomeWorks[i].getElementsByTagName("Paper_Code")[0].firstChild.nodeValue + "\" class=\"btn btn-default\">مشاهده</a></p></div></div>";
                }                
...
function GV_Order(v) {
    ...
    var t = document.getElementById("ContentPlaceHolder1_AllTags").innerHTML;
    var url = "http://localhost:23524/Ajax/GV_Order$!$" + t + "$!$" + v;
    ...
    xmlhttp.onreadystatechange = function () {
        ...
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            if (HomeWorks.length > 0) {
                ...
                for (i = 0; i < HomeWorks.length; i++) {
                    C += "<div class=\"col-xs-6 col-sm-3\"><div class=\"thumbnail\" align=\"center\"><img src=\"http://localhost:23524/images/MS-Word-icon.png\" class=\"img-thumbnail\" /><div class=\"caption text-right\"><div style=\"height:60px;\"><h3>" + HomeWorks[i].getElementsByTagName("TF_Name")[0].firstChild.nodeValue + "</h3>";
                    C += "</div><div class=\"scrollbar\" style=\"height:100px;\"><p>" + HomeWorks[i].getElementsByTagName("P_Desc")[0].firstChild.nodeValue + "</p></div></div><p><a href=\"#\" class=\"btn btn-primary\">خرید</a> <a href=\"View/" + HomeWorks[i].getElementsByTagName("Paper_Code")[0].firstChild.nodeValue + "\" class=\"btn btn-default\">مشاهده</a></p></div></div>";
                }
...
function SettlementRequest() {
    ...
    var url = "http://localhost:23524/Ajax/SettlementRequest$!$" + AB + "$!$" + RP + "$!$" + RD;
    ...
    xmlhttp.onreadystatechange = function () {
        ...
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            if (SettlementRequest.length > 0) {
                ...
                document.getElementById("lblErrorsDiv").style.display = 'block';
                document.getElementById("ContentPlaceHolder1_lblErrors").innerHTML = "مشکلی در انجام درخواست شما وجود دارد. لطفا کمی بعد مجددا اقدام کنید";
                $("#lblErrorsDiv").fadeOut(5000);
            }
            WFFToggle("FullPanel2", "WFF2");
        }
...
function SettleToggle() {
    var t = document.getElementById("FullPanel2");

    if (t.style.display == "" || t.style.display == "block") {
        document.getElementById("btnCol").setAttribute("class", "glyphicon glyphicon-plus");
        document.getElementById("FullPanel2").style.display = "none";
    } else {
        document.getElementById("btnCol").setAttribute("class", "glyphicon glyphicon-minus");
        document.getElementById("FullPanel2").style.display = "";
    }
}
...
function SupportDet(v) {    
    ...
    ifr.setAttribute("src", "../SupportView/" + v);
...
function RemoveHW() {
    ...
    var url = "http://localhost:23524/Ajax/RemoveHW$!$" + v;
    var xmlhttp;
    var txt, xx, x, i;
    //sending input data az url and the url will respond the out put as xml
    if (window.XMLHttpRequest) {// code for IE7+, Firefox, Chrome, Opera, Safari
...
function SettleDetAd(v) {
    ...
    //document.getElementById("myModal").modal('show');
    var ifr = document.getElementById("DetFrame");
    ifr.setAttribute("src", "../SettlementView/" + v);
...
//To cancle Settlement Request
function CancleSRShow(t) {
    document.getElementById("code").innerHTML = t;
    document.getElementById("ModalMessage").innerHTML = "آیا مایل به لغو نمودن این درخواست هستید؟";
    $("#myModal").modal('show');
}
...
function CancleSRDo() {
    ...
    var v = document.getElementById("code").innerHTML;
    var url = "http://localhost:23524/Ajax/CancleSR$!$" + v;
    var xmlhttp;
    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            if (state == "true") {
                document.getElementById("ModalMessage").innerHTML = "درخواست مورد نظر لغو شد.";
                UASR_Search('1');
            } else {
                document.getElementById("ModalMessage").innerHTML = "مشکلی در لغو این درخواست بوجود آمده است، لطفا مورد را از قسمت پشتیبانی به مدیریت بازار اطلاع دهید.";
            }
            setTimeout(function () { $("#myModal").modal('hide'); }, 2000);
        }
    }
...
function USMobileVerification() {
    
    var N = document.getElementById("ContentPlaceHolder1_txtMobile");    
    N.disabled = true;    
    document.getElementById("ContentPlaceHolder1_btnMobileVerify").disabled = true;
    document.getElementById("lblMessage").innerHTML = "در حال ارسال کد فعالسازی برای شماره موبایل شما...";
    $("#lblMessage").fadeIn();
    $("#lblMessage").fadeOut(3000);
    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            if (state == "ارسال با موفقیت انجام شده است") {
                $("#lblMessage").hide();
                document.getElementById("lblMessage").innerHTML = "کدی 4 رقمی برای شماره همره شما ارسال شده است. لطفا آنرا در زیر وارد کنید";
                $("#lblMessage").fadeIn();
                $("#lblMessage").fadeOut(3000);
                document.getElementById("ContentPlaceHolder1_lblMobileVerification").style.display = 'none';
                document.getElementById("SVC").style.display = 'block';
                $("#ContentPlaceHolder1_txtVerificationCode").mask("9999");

            } else {
                $("#lblMessage").hide();
                document.getElementById("lblMessage").innerHTML = "مشکلی در ارسال پیام برای گوشی همراه شما وجود دارد. لطفا بعدا مجددا فرایند را ادامه دهید.";
                $("#lblMessage").fadeIn();
                $("#lblMessage").fadeOut(3000);
            }
