...
public partial class Default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        LoginClass LC = new LoginClass(new Users("635569326832807388"));
        ...
        if (BNews!=null && BNews.Count > 0)
        {
            ...
            for (int i = 0; i < Math.Min(4, BNews.Count); i++)
            {
                ...
                StrNews += "<p><a href=\"#\" class=\"btn btn-primary\">خرید</a> <a href=\"../View/" + Encryption.Encrypt(BNews[i].Paper_Code,SharedDataInfo.SecurityKey) + "\" class=\"btn btn-default\">مشاهده</a></p>";
                StrNews += "</div></div></div>";
            }
            StrNews += "</div>";
        }
    ...
    private List<Homework> LoadBatch(ViewType T, int TCount)
    {
        ...
        switch (T)
        {
            ...
            case ViewType.News:                
                ...
                sqlCmd = "SELECT *  FROM [dbo].[PaperList]";
                DataTable DT = mySql.SqlExecuteReader(sqlCmd,new SqlParameter[0]);
