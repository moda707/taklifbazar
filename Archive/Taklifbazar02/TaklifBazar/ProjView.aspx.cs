        ...
        if (!IsPostBack)
        {
            try
            {
                if (!String.IsNullOrEmpty(Request.QueryString[0]))
                {
                    ...
                    QS = QS.Replace(' ', '+');
                    //int mod4 = QS.Length % 4;
                    //if (mod4 > 0)
                    //{
                    //    QS += new string('=', 4 - mod4);
                    //}
                    ...
                    var p = new SqlParameter("@Paper_Code", SqlDbType.NVarChar);
                    p.Value = HW.Paper_Code;
                    parameters.Add(p);


                    sqlCmd = "SELECT T.*  FROM [dbo].[FileList] T WHERE T.Paper_Code = @Paper_Code ";
                    txtPrevFiles = "";
                    ...
                    {
                        string k = r[13].ToString() + "!" + DateTime.Now.Ticks.ToString();
                        string script = "javascript:RedirectDownload('" + Encryption.Encrypt(k, SharedDataInfo.SecurityKey) + "');";
