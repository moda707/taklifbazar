...
public partial class DoAction1 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        switch (QS_Action)
        {
            ...
            case "CancleSR":
                ...
                tb_SettlementRequest S = new tb_SettlementRequest(Code);
                S.Status = SettlementReq_Status.Canceled;
            ...
            case "Ad_TN_Search":
                ...
                break;
            case "ShowMore":                
                string[] _Tags = QS[1].Split('!');
                ViewType _VT = (ViewType)Convert.ToInt32(QS[2]);
                int _StartAt = Convert.ToInt32(QS[3]);
            ...
            case "SettlementRequest":
                Int32 AccBal = Convert.ToInt32(QS[1].Replace(",",""));
                int RP = Convert.ToInt32(QS[2]);
                int RD = Convert.ToInt32(QS[3]);
    ...
    public string SR2XML(List<tb_SettlementRequest> SR, string C, string P)
    {
        ...
        foreach (tb_SettlementRequest a in SR)
        {
            ...
            XMLPack += "<StNum>" + ((int)a.Status).ToString() + "</StNum>";
            XMLPack += "</SR>";
        }
        XMLPack += String.Format(@"<Pagination><Count>{0}</Count><Page>{1}</Page></Pagination>", C, P);
        XMLPack += "</CATALOG>";
