...
public partial class UAccount : System.Web.UI.Page
{
        ...
        if (!Page.IsPostBack)
        {
            ...
            if (LSR.Count > 0)
            {
                ...
                foreach (tb_SettlementRequest a in LSR)
                {
                    ...
                    C += "<tr><td>" + a.Pre_Req_Code + "</td><td>" + DateConversion.DateTimeToPersian(D) + "</td><td>" + D.Hour + ":" + D.Minute + "</td><td>" + a.Subject + "</td><td><span" + tb_SettlementRequest.GetSRStatusNamelbl(a.Status) + "</span></td><td><button type=\"button\" class=\"btn btn-link\" onclick=\"javascript:RemoveSR('SReq_" + a.RN + "')\">لغو</button></td></tr>";
                }
                C += "</tbody></table>";
            }//
            else
    ...
    protected void btnSaveRequest_Click(object sender, EventArgs e)
    {
        ...
        DateTime ND = DateTime.Now.AddDays(RD);
