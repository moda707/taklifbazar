...
public partial class UProjects : System.Web.UI.Page
{
    ...
    protected void DataBind()
    {
        ...
        {
            C =
                "<table class=\"table table-striped\"><thead><tr><th>#</th><th>عنوان</th><th>توضیحات</th><th>قیمت</th><th>فروش</th><th></th><th></th></tr></thead><tbody>";
            foreach (Homework a in LHW)
            {
                ...
                C += "<tr><td>" + a.RN + "</td><td>" + a.TF_Name + "</td><td>" + Description + "</td><td>" +
                     NumericClass.int2Currency(Convert.ToInt32(a.Price)) + "</td><td>" + a.Download_C +
                     "</td><td><button type=\"button\" class=\"btn btn-link\" onclick=\"javascript:HWDet('" +
                     Encryption.Encrypt(a.Paper_Code, SharedDataInfo.SecurityKey) +
                     "');\">جزئیات</button></td><td><button type=\"button\" class=\"btn btn-link\"  onclick=\"javascript:HWRM('" +
                     Encryption.Encrypt(a.Paper_Code, SharedDataInfo.SecurityKey) +
                     "');\" >حذف</button></td></tr>";
            }
            C += "</tbody></table>";
