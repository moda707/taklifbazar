            ...
            try
            {
                if (!String.IsNullOrEmpty(Request.QueryString[0]))
                {
                    ...
                    //if (Mode == "-1")
                    {
                        ...
                        string Mode = q.Split('!')[0];
                        string[] Tags = q.Split('!')[1].Split('-');
                        AllTags.InnerHtml = q.Split('!')[1].ToString();
                        HWShowClass HWSC = new HWShowClass();
                        List<Homework> LSH = Homework.SearchByTag(Tags, ViewType.News, 1, 8);
                        string TagStr = "";
                        foreach (string k in Tags)
                        {
                            TagStr += "<span class=\"label label-default\" style=\"font-size:9pt;\">" + k.Trim() + "</span>&nbsp;&nbsp;";
                        }
                        TagsShow.InnerHtml = TagStr;
                        string GV = txtGV.InnerHtml;
                        foreach (Homework a in LSH)
                        {
                            GV += "<div class=\"col-xs-6 col-sm-3\"><div class=\"thumbnail\" align=\"center\"><img src=\"http://localhost:23524/images/MS-Word-icon.png\" class=\"img-thumbnail\" /><div class=\"caption text-right\"><div style=\"height:60px;\"><h3>" + a.TF_Name + "</h3>";
