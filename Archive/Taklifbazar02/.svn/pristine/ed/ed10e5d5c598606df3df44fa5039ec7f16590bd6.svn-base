...
public class Admin_ProjectClass
{
    ...
    public static List<Admin_ProjectClass> SearchByTag(string[] T, Project_List_OrderBy OrderBy, List_OrderType OrderType)
    {
        ...
        try
        {
            ...
            sqlCmd1 = "SELECT T.Paper_Code, T.Data3 Pre_Paper_Code, T.TF_Name, T.TE_Name, T.Paper_Rate, T.Download_C, T.Price, F.User_Code, (F.FFName + ' ' + F.FLName) Name, T.Upload_Date  FROM [PaperBank].[dbo].[PaperList] T LEFT JOIN [PaperBank].[dbo].[Users]  F ON F.User_Code=T.Owner_Code " + tmpTags + _OrderBy;
