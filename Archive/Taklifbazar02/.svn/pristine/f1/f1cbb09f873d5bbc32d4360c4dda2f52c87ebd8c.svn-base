...
public partial class UserPortal : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            sqlCmd = string.Format(@"DECLARE @Code BIGINT = {0} SELECT ISNULL((SELECT T.Amount FROM dbo.Financial_Acc T WHERE T.User_Code = @Code),0) AccBall, COUNT(T1.Download_C) ProjCount, SUM(T1.Download_C) SellCount, ISNULL((SELECT SUM(T2.Amount) FROM dbo.Transactions T2 WHERE T2.User_Code = @Code AND T2.Trans_Type = 1),0) SellPrice FROM dbo.PaperList T1 WHERE T1.Owner_Code = @Code", "@User_Code");

            List<SqlParameter> parameters = new List<SqlParameter>();
            var p = new SqlParameter("@User_Code", SqlDbType.NVarChar);
            p.Value = UPC.CurUser.User_Code;
            parameters.Add(p);

            DataTable DT = mySql.SqlExecuteReader(sqlCmd,parameters.ToArray());
            if (DT.Rows.Count > 0)
            {
                txtAccBal = NumericClass.int2Currency(Convert.ToInt32(DT.Rows[0][0].ToString()));
                txtProjCount = DT.Rows[0][1].ToString();
                txtTotalSellCount = DT.Rows[0][2].ToString();
                txtTotalSellPrice = NumericClass.int2Currency(Convert.ToInt32(DT.Rows[0][3].ToString()));
            }
        }
