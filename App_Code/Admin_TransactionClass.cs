...
public class Admin_TransactionClass
{
    ...
    public static List<Admin_TransactionClass> LoadByUser(string _User_Code, Transaction_List_ShowType _TLS)
    {
        ...
        try
        {
            ...
            string WhStr = "";
            string WhStr1 = "";
            if (_User_Code != "") WhStr1 = " T.User_Code = '" + _User_Code + "' ";
            ...
            sqlCmd1 = String.Format(@"SELECT T.Transaction_Code, T.Trade_Code, T.User_Code, ISNULL((F.FFName + ' ' + F.FLName),' ') Name, T.Amount, T.Trans_Subject, T.CurAccount_Type, T.InOut, T.BN_Portal, T.BN_Source, T.Trans_Type, F.Pre_User_Code FROM [PaperBank].[dbo].[Transactions] T LEFT JOIN [PaperBank].[dbo].[Users] F ON F.User_Code = T.User_Code {0}", WhStr);
