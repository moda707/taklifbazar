        ...
        try
        {
            ...
            if (HasTag)
            {
                ...
                foreach (string a in Tags)
                    if (a.Trim() != "")
                        WhereStr = "(T0.FFName + ' ' + T0.FLName) LIKE N'%" + a + "%' OR (T0.EFName + ' ' + T0.ELName) LIKE N'%" + a + "%' OR T0.User_Code LIKE N'%" + a + "%' OR ";
            ...
            sqlCmd1 = String.Format(@"SELECT T0.User_Code, T0.FFName, T0.FLName, T0.EFName, T0.ELName, T1.Amount AccBal, T2.HW_Count, T3.HW_Sell, T4.HW_Buy
                                        FROM [PaperBank].[dbo].[Users] T0
                                        LEFT JOIN [PaperBank].[dbo].[Financial_Acc] T1 ON T1.User_Code = T0.User_Code
                                        LEFT JOIN (SELECT R.Owner_Code UCode, COUNT(R.Download_C) HW_Count FROM [PaperBank].[dbo].[PaperList] R GROUP BY R.Owner_Code) T2 ON T2.UCode = T0.User_Code
                                        LEFT JOIN (SELECT SUM(K.Amount) HW_Sell, K.User_Code FROM [PaperBank].[dbo].[Transactions] K WHERE K.Trans_Type=1 GROUP BY K.User_Code) T3 ON T3.User_Code = T0.User_Code
                                        LEFT JOIN (SELECT SUM(K.Amount) HW_Buy, K.User_Code FROM [PaperBank].[dbo].[Transactions] K WHERE K.Trans_Type=0 GROUP BY K.User_Code) T4 ON T4.User_Code = T0.User_Code
                                        {0}
